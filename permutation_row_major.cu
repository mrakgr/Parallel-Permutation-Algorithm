...
__device__ unsigned long inline get_element(unsigned long *per_m, int row, int col, int factor){
	if (row < 0) return 0;
	unsigned long pos = factor*row + col;
	return per_m[pos];
}

__device__ void inline set_element(unsigned long *per_m, int row, int col, int factor, unsigned long value){
	unsigned long pos = factor*row + col;
	per_m[pos] = value;
}
...
void print_matrix_map(device_vector<unsigned long> matrix_map, unsigned long limit, unsigned long factor){
	for (unsigned long i = 0; i <= (limit - 1)*factor; i += factor){
		// Prints only the first 40 columns so the screen does not overflow.
		thrust::copy(matrix_map.begin() + i, matrix_map.begin() + std::min(factor, 40UL) + i, ostream_iterator<unsigned long>(cout, " "));
		cout << endl;
	}
}
...
__global__ void createPermutationMatrix_kernel(unsigned long *per_m, unsigned long factor, unsigned long per_num, int row, unsigned long factor2){
	...
	if (col < factor){
		...
		m_previous = get_element(per_m, row - 1, col, factor); // Get the element of the previous row.
		...
		set_element(per_m, row, col, factor, new_m); // matrix_map(row,column) is set to new_m.
	}
}
...
__global__ void computeIndices_kernel(unsigned long *per_m, unsigned long factor, unsigned long per_num, int row){
	...
	if (col < factor){
		...
		m_previous = get_element(per_m, row - 1, col, factor);
		m_current = get_element(per_m, row, col, factor);
		...
		set_element(per_m, row, col, factor, index_of_new_m);
	}
}
...
int main(){
	...
	// On my GTX 970 a thousand iterations of generating 10! matrices gives results at 6.04 seconds. For comparison, the STL iterator would 
	// need 1.7 secondson my OC'd i-4690k to compute a single matrix. The GPU version is roughly 280 times faster at generating permutation
	// matrices.

	// The row major version of the algorithm as it has coalesced memory accesses is much faster than the column major algorithm.
